...
import { initializeBrowser, closeBrowser, getPage, getBrowser } from "./page.config.js";
...
const initializeSession = async () => {
  try {
    ...
    try {
      ...
      console.error("Error cerrando browser después de fallo:", closeError.message);
    }
...
cron.schedule("*/3 * * * *", async () => {
  try {
    ...
    console.log(`Estado de inicialización: ${isInitialized ? 'INICIALIZADO' : 'NO INICIALIZADO'}`);

    if (!isInitialized) {
      console.log("🔄 Reintentando inicialización...");
      const success = await initializeSession();
      if (!success) {
        console.log("❌ No se pudo inicializar la sesión, reintentando en el próximo ciclo (3 minutos)");
        return;
      }
    }
    ...
    try {
      ...
      isInitialized = false;
      console.log("🔄 Sistema reiniciado por error. Se volverá a inicializar en el próximo ciclo (3 minutos).");
    }
    ...
    isInitialized = false;
    console.log("💥 Error crítico manejado. El cron continuará funcionando y reintentará en 3 minutos.");
  }
});
